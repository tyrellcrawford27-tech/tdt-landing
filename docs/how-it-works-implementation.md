...
Phones show the product above the chat. A compact four-topic bar sticks below the site's header and respects the top safe area. Every topic has a touch target larger than 44px, a numbered marker and a short label. The current step's full title and description appear below the controls in normal document flow, so narrow screens and larger text cannot collide with the product. Previous/Next buttons below the scene also select topics and return to the start of the scene with native scrolling. Phones, very short desktop viewports and reduced-motion mode retain one full-height panel with manual step controls; they do not stack duplicate panels or force the scene into the screen. Mobile topic controls remain sticky with reduced motion. No wheel or touch gesture is trapped.

On phones, the conversation and community cards determine their own heights instead of sitting in a fixed 440px interaction area. The complete initial conversation is visible without an inner scroll. Once the visitor adds messages, the chat history is capped at 420px and follows new replies. Product screenshots retain the exact 1500:1385 aspect ratio. The video selector and send control have 44px touch targets; the composer uses a 16px input and the keyboard's Send action. Profile overlays ignore touch pointer-leave events, preventing touch release from dismissing a newly opened profile, and include a mobile close button. They stay below the fixed site header and below the topic controls when there is room; on short landscape screens a compact profile can overlap the topic bar without its content or close button being hidden behind it.
