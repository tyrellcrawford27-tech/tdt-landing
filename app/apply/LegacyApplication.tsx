...
// Frozen v4 questions for applications started before the v5 rollout.
// Do not migrate these answers or use the current form-version constant here.
...
import { applicationWrittenAnswerError } from '@/lib/applicationForm';
...
// ── Page ──────────────────────────────────────────────────────────────────────
function ApplyPageInner({ onStartNew }: { onStartNew: () => void }) {
  ...
  const validateContent = (field: keyof FormData, v: string): boolean => {
    if (
      ['full_name', 'guardian_name', 'current_team_school', 'biggest_weakness', 'goal', 'city_state'].includes(field) &&
      applicationWrittenAnswerError(v)
    ) return true;
...
export default function LegacyApplication({ onStartNew }: { onStartNew: () => void }) {
  return (
    <Suspense>
      <ApplyPageInner onStartNew={onStartNew} />
    </Suspense>
  );
}
