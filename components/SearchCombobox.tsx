...
// The shared autocomplete engine behind LocationInput and SchoolInput.
// One implementation owns the interaction layer — synchronous search on every
// keystroke, animated panel height, staggered rows, full combobox ARIA, the
// Enter-vs-form-advance contract, and the touch selection model — so the two
// fields can never drift apart in feel.
...
export type ComboRow = {
  ...
  primary: string;       // left text in the row
  qualifier: string;     // dimmed right text
  tier: number;          // >= 5 means Enter with no highlight may auto-commit
  matchStart: number;    // highlight range in `primary`; -1 = none
  matchLen: number;
};

export type ComboStatus = 'idle' | 'loading' | 'ready' | 'failed';
...
type Props = {
  ...
  placeholder: string;
  inputName: string;     // unusual on purpose — suppresses browser autofill panels
  hintText: string;      // visually-hidden usage hint for screen readers
  listLabel: string;     // aria-label of the listbox
  noMatchCopy: string;   // row shown when the index is ready but finds nothing
  loadingCopy: string;   // row shown when the index is still loading after 500ms
  failedCopy: string;    // row shown when the index failed to load
  noMatchAnnounce: string;
  /** Synchronous search. Return null while the index is not ready yet. */
  search: (q: string) => ComboRow[] | null;
  preload: () => Promise<unknown>;
  getStatus: () => ComboStatus;
};
...
export const SearchCombobox = forwardRef<HTMLInputElement, Props>(
  function SearchCombobox(props, ref) {
    const {
      value, onChange, baseStyle, onCommit,
      placeholder, inputName, hintText, listLabel,
      noMatchCopy, loadingCopy, failedCopy, noMatchAnnounce,
      search, preload, getStatus,
    } = props;
    ...
    const [status, setStatus]       = useState<ComboStatus>(() => getStatus());
    ...
    // Load the index. The loaders are module-scoped, so this is a no-op after
    // the first call anywhere in the app — going back a screen and forward
    // again is free.
    useEffect(() => {
      ...
      preload().then(() => {
        ...
        if (pending && document.activeElement === inputRef.current) {
          const next = search(pending);
          if (next) { setRows(next); setOpen(next.length > 0); }
        }
      });
      return () => {
      ...
      // preload/search/getStatus are module-backed and stable per wrapper.
      ...
      if (next) {
        ...
        if (!slowTimer.current) {
          slowTimer.current = setTimeout(() => {
            if (mounted.current && !search(v)) { setSlowLoad(true); setOpen(true); }
          }, 500);
        }
      }
    };
    ...
    const reopenOnFocus = () => {
      // After a commit, refocusing does not re-offer the thing the user just
      // chose. The next keystroke reopens normally.
    ...
    // 6 → 2 rows collapses rather than snapping. Driven as an explicit pixel
    // height (height:auto can't transition), measured in a layout effect keyed
    // to the content — NOT a ResizeObserver, whose callbacks are delivered per
    // rendering frame and never fire in a throttled or backgrounded tab.
    useLayoutEffect(() => {
        ...
        {showPanel && (
          ...
          >
              ...
              {rows.map((row, i) => (
                <li
                  ...
                  style={{
                    ...
                    // arrived rows animate in. That's what stops a fast typist
            ...
            {rows.length === 0 && slowLoad && status !== 'failed' && (
              <p style={HINT_ROW}>{loadingCopy}</p>
            )}
            {rows.length === 0 && status === 'failed' && (
              <p style={HINT_ROW}>{failedCopy}</p>
            )}
            {rows.length === 0 && status === 'ready' && query.trim() !== '' && (
              <p style={HINT_ROW}>{noMatchCopy}</p>
            )}
            </div>
          </div>
        )}
...
function highlight(row: ComboRow) {
  const s = row.primary;
  if (row.matchStart < 0 || row.matchLen <= 0) return s;
  const a = s.slice(0, row.matchStart);
  const b = s.slice(row.matchStart, row.matchStart + row.matchLen);
  const c = s.slice(row.matchStart + row.matchLen);
  return (
    <>
      {a}
